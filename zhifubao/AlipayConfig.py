...
class Config():
    def __init__(self):
        ...
        self.partner = "2088721367731375"
        # 交易安全检验码，由数字和字母组成的32位字符串
        self.key = "o0f7yua35rrxa9jj881h63i92f0pvx20"
        # 签约支付宝账号或卖家支付宝帐户
        self.seller_id = "470235676@qq.com"
        # 页面跳转同步返回页面文件路径 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
        self.return_url = "http://relalive.com/zhifubao/alipy_notify/"
        # 服务器通知的页面文件路径 要用 http://格式的完整路径，不允许加?id=123这类自定义参数
        self.notify_url = "http://relalive.com/zhifubao/zfb/"
